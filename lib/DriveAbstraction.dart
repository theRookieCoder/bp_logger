...
class DriveAbstraction {
  ...
  static Future<String> getFileId(drive.DriveApi driveApi, String fileName) async {
    ...
    final search = await driveApi.files.list(q: "name='$fileName' and trashed=false", spaces: 'drive'); // Use name to search and make sure it isn't in the bin
  ...
  static Future<List<List<int>>> getFileData(drive.DriveApi driveApi, String fileID) async {
    drive.Media fileMedia = await driveApi.files.export(fileID, "text/csv", downloadOptions: drive.DownloadOptions.FullMedia); // Get contents of a Google Sheets file in csv format
    final dataStreamList = await fileMedia.stream.toList(); // Receive the Stream as List
  ...
  static Future<void> appendToFile(drive.DriveApi driveApi, String fileID, String text, List<int> filteredDataStreamList) async {
    ...
    final dataString = ascii.decode(filteredDataStreamList); // Decode List<int> to a String

    var placeholder = new drive.File(); // Placeholder file for update()
    Stream<List<int>> mediaStream = Future.value(List.from(ascii.encode("$dataString\n$text")).cast<int>().toList()).asStream().asBroadcastStream(); // Add existing data and new text data, then make it into a Stream<List<int>> again
    ...
    await driveApi.files.update(placeholder, logFileID, uploadMedia: media); // Update the existing file with the new data
  }
}
